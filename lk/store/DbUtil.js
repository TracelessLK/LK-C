...
const updateSqlObj = {
  ...
  '0.0.5': `alter table record add column audioDuration INTEGER`
}
...
create table if not exists db_version(
  ...
  primary key(version)
        ...
        const sqlAry = [
          "create table if not exists chat(id TEXT,ownerUserId TEXT,name TEXT,createTime INTEGER, isGroup INTEGER,reserve1 TEXT,MessageCeiling INTEGER,focus INTEGER,PRIMARY KEY(ownerUserId,id))",
...
(
   ...
   ifnull(t2.sendTime,t1.createTime) as activeTime,
   t1.createTime,
   t1.isGroup,
   t1.MessageCeiling,
   t1.focus,
  ...
  case when t2.senderUid = t2.ownerUserId then "我" else t4.name end ||": " ||(case t2.type when 0 then replace(replace(trim(t2.content),"\n"," "), "&nbsp;", " ") when 1 then "[图片]" when 2 then "[文件]" when 3 then "[语音]" end)  as content,
   t3.pic,
   sum(t2.readState<1 and t2.senderUid <> t1.ownerUserId  and t2.ownerUserId = t1.ownerUserId ) as newMsgNum
   from
   chat as t1
