...
class MagicCodeManager extends EventTarget {
  ...
  async asyGetOrgMCode() {
    if (!this._orgMCode) {
      const mc = await LKMagicCodeProvider.asyGetMagicCode(Application.getCurrentApp().getCurrentUser().id)
      if (mc) { this._orgMCode = mc.orgMCode }
    }
  ...
  async asyGetMemberMCode() {
    if (!this._memberMCode) {
      const mc = await LKMagicCodeProvider.asyGetMagicCode(Application.getCurrentApp().getCurrentUser().id)
      if (mc) { this._memberMCode = mc.memberMCode }
    }
